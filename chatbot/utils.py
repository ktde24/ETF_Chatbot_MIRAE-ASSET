...
def validate_user_profile(user_profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(level, str):
        ...
        validated['level'] = int(level) if level in [1, 2, 3] else 1
    
    # 투자자 유형 검증
    investor_type = validated.get('investor_type', 'ARSB')
    if investor_type not in ['ARSB', 'ARSE', 'ARTB', 'ARTE', 'AETB', 'AETE', 'AESB', 'AESE',
                            'IRSB', 'IRSE', 'IRTB', 'IRTE', 'IETB', 'IETE', 'IESB', 'IESE']:
        validated['investor_type'] = 'ARSB'
